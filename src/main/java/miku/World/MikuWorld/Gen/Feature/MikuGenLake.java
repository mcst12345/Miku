...
public class MikuGenLake extends WorldGenerator {
    private final Block block;

    public MikuGenLake(Block blockIn) {
        this.block = blockIn;
    }
                ...
                for (int j3 = 0; j3 < 16; ++j3) {
                    for (int j4 = 4; j4 < 8; ++j4) {
                        if (aboolean[(i2 * 16 + j3) * 8 + j4]) {
                            ...
                            if (worldIn.getBlockState(blockpos).getBlock() == BlockLoader.MikuDirt && worldIn.getLightFor(EnumSkyBlock.SKY, position.add(i2, j4, j3)) > 0) {

                                worldIn.setBlockState(blockpos, BlockLoader.MikuGrass.getDefaultState(), 2);
                            }
                        }
                    }
                }
                    ...
                    for (int k3 = 0; k3 < 16; ++k3) {
                        for (int k4 = 0; k4 < 8; ++k4) {
                            ...
                            if (flag1 && (k4 < 4 || rand.nextInt(2) != 0) && worldIn.getBlockState(position.add(j2, k4, k3)).getMaterial().isSolid()) {
                                worldIn.setBlockState(position.add(j2, k4, k3), BlockLoader.MikuStone.getDefaultState(), 2);
                            }
                        }
                    }
                }
